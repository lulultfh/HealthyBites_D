...
<head>
    ...
    <title>Order Details - HealthyBites</title>
    <!-- Google Web Fonts -->
...
<div class="container-fluid fixed-top">
    ...
    <div class="container-fluid page-header py-5">
        <h1 class="text-center text-white display-6">Order #<?= $order['id'] ?></h1>
    </div>
        ...
        <div class="row g-5">
            <div class="col-lg-12">
                <div class="bg-light rounded p-5">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p><strong>Status:</strong> 
                                <span class="badge bg-<?= $order['status'] == 'completed' ? 'success' : ($order['status'] == 'shipped' ? 'primary' : ($order['status'] == 'processing' ? 'warning' : 'secondary')) ?>">
                                    <?= ucfirst($order['status']) ?>
                                </span>
                            </p>
                            <p><strong>Payment Method:</strong> <?= ucfirst(str_replace('_', ' ', $order['payment_method'])) ?></p>
                            <p><strong>Shipping Method:</strong> <?= $order['shipping_method'] == 'free' ? 'Free Shipping' : 'Flat Rate (Rp15,000)' ?></p>
                            
                            <h4 class="mt-5 mb-4">Shipping Information</h4>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="bg-white p-4 rounded">
                                <div class="d-flex justify-content-between py-2">
                                    <span>Subtotal:</span>
                                    <span>Rp<?= number_format($order['total_amount'] - ($order['shipping_method'] == 'flat' ? 15000 : 0), 0, ',', '.') ?></span>
                                </div>
                                <div class="d-flex justify-content-between py-2">
                                    <span>Shipping:</span>
                                    <span>Rp<?= $order['shipping_method'] == 'flat' ? '15,000' : '0' ?></span>
                                </div>
                    ...
                    <div class="mt-5">
                        <h4 class="mb-4">Order Status History</h4>
                        <div class="timeline">
                            <?php while ($status = $status_history->fetch_assoc()): ?>
                            <div class="timeline-item">
                                <div class="timeline-marker"></div>
                                <div class="timeline-content">
                                    <p class="fw-bold"><?= ucfirst($status['status']) ?></p>
                                    <p class="text-muted small"><?= date('F j, Y g:i A', strtotime($status['created_at'])) ?></p>
                                </div>
                            </div>
                            <?php endwhile; ?>
                        </div>
                    ...
                    <div class="text-center mt-5">
                        <a href="add_testimonial.php?order_id=<?= $order['id'] ?>" class="btn btn-primary px-4 py-2">
                            Add Testimonial
                        </a>
                    </div>
                    <?php endif; ?>
